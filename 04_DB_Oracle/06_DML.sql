/*
    ...
    - 테이블의 특정 컬럼에 대한 값만 INSERT 하고자 할 떄 사용
    -선택 안된 컬럼들은 기본적으로 NULL 값이 들어감
    - 기본값(DEFAULT)이 저장되어 있으면 기본값이 들어감
    
    INSERT INTO 테이블명 (서브쿼리);
    - VALUES 대신 서브쿼리로 조회한 겨로가값을 통째로 INSERT
...
CREATE TABLE EMP_DML (
    ...
    -- EMP_NAME 컬럼에 NOT_NULL 제약조건에 의해 에러
    ...
    INSERT INTO EMP_DML (
        ...
        FROM EMPLOYEE 
        JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID) --JOIN 여러테이블을 연결
    );
...
--EMPLOYEE에서 EMP_NAME이 선동일인 사람의 급여(SALARY)를 700원, 보너스를 0.2로 변경

UPDATE EMPLOYEE 
SET SALARY = 700, BONUS = 0.2
WHERE EMP_NAME ='선동일';
SELECT * FROM EMPLOYEE;
...
/*
    DELETE
    테이블에 기로고딘 데이터를 삭제하는 구문
    
    DELETE FROM 테이블명
    WHERE 조건식; 기본키(PRIMARY KEY) =값;
    
    ---> WHERE 절을 제시하지 않으면 전채 행이 삭제됨
