/*
    ...
    예시) ATM기
        ...
        5. 현금 인출
        6. 완료
    - 각각의 업무들을 묶어서 하나의 작업 단위로 만드는 것을 트랙젝션이라고 한다.
    
    COMMIT : 모든 작업들을 정상적으로 처리하겠다고 확정
    ROLLBACK : 모든 작업들을 취소하겠다고 확정 (마지막 COMMIT)
    SAVEPONIT : 임시 저장 시점을 정하고 ROLLBACK 진행 시 SAVEPOINT까지 일부 롤백
    ...
    */
    ...
    --USER_03 에서 USER_ID가 10, 20인 사람들 삭제
    
    DELETE USER_03
    WHERE USER_ID IN (10, 20);
    
    -- 두행이 삭제된 시점에서 SAVEPONIT 지정
    SAVEPOINT SP;
