//태그로 가져오기 = 같은 태그로 여러개일 수 잇으므로 배열로 반환
function btn1() {
...
function btn6() {
  ...
  divs[0].textContent = "<span>안녕하세요</spen>"; //태그를 문자로 출력
  divs[1].innerHTML = "<spen> 안녕하세요 </spen>"; //태그를 HTML로 출력
  console.log(divs[1].textContent);
}

function btn7() {
  const divs = document.querySelector("#testId");
  //속성 추가 & 수정
  divs.setAttribute("data-test", "테스트"); //(1)속성명, (2)속성값
  // 속성 값 가져오기
  console.log(divs.getAttribute("data-test"));
...
const div2 = document.querySelector("#testId2"); //전역에서 선택 전체영역
function btn9() {
...
function btn13() {
  ...
  const p = document.createElement("p"); //태그 생성
  p.textContent = "텍스트 추가"; // p 태그 안에 텍스트 내용 추가
  div.appendChild(p); //div에 추가
}
