/*
 * * 자바의 특징
 * 1. 객체지향 프로그래밍(oBJECT-oRIENTED pROGRANNUBG, OOP) 언어
 * 		- 모든 기능을 "객체" 라는 단위로 구성*
 * 		- 캡슐화, 상속, 다형성, 추상화
 ...
 *		-  JRE (Java Runtime Environment) : 자바 샐행 환경
 ...
 * 		3. 번역된 바이트코드(기계어)를 JVM의 자바 인터프리터(javac.exe)를 이용해 실행한다.
 * 
 * 
 * 
 * 
 ...
 * 		클래스를 저장하고 관리한다.
 * 		- 패키지는 클래스를 유일하게 만들어주는 식별자 역화을 한다.
 * 		- 패키지가 선언되지 않으면 ddefault 패키지에 속하게 되는데 실무에서는 거의 사용하지 않는다.
 * 
 * */
...
public class Hello { // 클래스명은 클래스 파일명과 동일해야 한다.
	 ...
	 * - 해당 메서드 호풀 시 메서드이 블럭({})에 해당하는 부분을 실행한다.
	 * */
	...
	public static void main(String[] args) {
		...
		/*
		 * 네이밍 규칙
		 *  - 패키지명 : 소문자로 작성, 회사들 간에 패키지가 서로 중복되지 않도록 회사 도메인 이름으로 패키지를 만든다.
		 * 	- 클래스명 : 대문자로 시작, 여러 개의 단어가 있을 때에는 단어 앞자리마다 대문자
		 *  - 매서드명 : 소문자로 시작, 여러 개의 단어가 있을 때에는 단어 앞자리마다 대문자
		 * 
		 * */
