/*
	LIMCT
	- OTHER BY 절보다 뒤에 조건을 걸고 싶을 때 사용
    - 출력되는 행 수를 제한하는 MYSQL 전용 비표준 구문
    - 데이터 양을 제한하고자 할 떄 유용 (페이징 처리)
*/
SELECT * FROM actor
ORDER BY first_name
-- LIMIT 10 OFFSET 20;
...
/*
	날짜 퍼리 함수
   * NOW(), CURRENT_TIMESTAMP() : 현제 날짜와 시간 반환
    CURDATE(), * CURRENT_DATE() : 현제 널짜 반환
    CURTIME, CURRENT_TIME() : 현제 시간 반환
...
/*
    ...
    DATE_FORMAT(날짜, 포맷방식) : 날짜 형식을 변경해서 반환

*/
SELECT 
	FORMAT(amount, 0),
    DATE_FORMAT(patment_date,'%Y년 %m일 %d일 %H시 %i분 %s초'),
...
SELECT address, address2, IFNULL(address2, '주소없음')
 ...
 /*
	IF(값1, 값2, 값3) | IF(조건, 조건TURE인 경우, 조건 FALSE인 경우)
    - 값1이 NULL이 아니면 값2 반환, NULL이면 값3 반환
    - 조건애 해단하면 두번째 값 반환, 해당하지 않으면 마지막 값 반환
 */
 SELECT 
 IF(address2, '주소있음', '주소없음')
 IF(address LIKE '1%', '해당 값 ture', 'False')
 FROM address;
